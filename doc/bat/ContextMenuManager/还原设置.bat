...
cd /d "%~dp0"
:: =================================================================================================
set app_name=ContextMenuManager
...
if not exist "!backup%pos%!\Config" (
    echo 缺少配置目录 Config
    call :failed
)

md "%appdata%\%app_name%" >nul 2>&1
md "%appdata%\%app_name%\Config" >nul 2>&1
xcopy /e/y/i/f "!backup%pos%!\Config" "%appdata%\%app_name%\Config" >nul || call :failed
echo ------------------------------------------
