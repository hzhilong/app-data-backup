...
md "%dir%"
if not exist "%appdata%\%app_name%\Everything.ini" (
    echo 缺少配置文件 Everything.ini
    call :failed
)

copy /y "%appdata%\%app_name%\Everything.ini" "%dir%\" >nul 2>&1 || call :failed
copy /y "%appdata%\%app_name%\Bookmarks.csv" "%dir%\" >nul 2>&1 || call :failed
copy /y "%appdata%\%app_name%\Search History.csv" "%dir%\" >nul 2>&1 || call :failed
copy /y "%appdata%\%app_name%\Run History.csv" "%dir%\" >nul 2>&1 || call :failed
