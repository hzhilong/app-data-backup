...
if not exist "!backup%pos%!\Everything.ini" (
    echo 缺少配置文件 Everything.ini
    call :failed
)

md "%appdata%\%app_name%" >nul 2>&1
copy /y "!backup%pos%!\Everything.ini" "%appdata%\%app_name%\" >nul || call :failed
copy /y "!backup%pos%!\Search History.csv" "%appdata%\%app_name%\" >nul || call :failed
copy /y "!backup%pos%!\Run History.csv" "%appdata%\%app_name%\" >nul || call :failed
copy /y "!backup%pos%!\Bookmarks.csv" "%appdata%\%app_name%\" >nul || call :failed
echo ------------------------------------------
