...
md "%dir%"
if not exist "%installLocation%\FMStorage.list" (
    echo 缺少配置文件 FMStorage.list
    call :failed
)

copy /y "%installLocation%\FMStorage.list" "%dir%\" >nul 2>&1 || call :failed
xcopy /e/y/i/f "%installLocation%\Storage" "%dir%\Storage" >nul 2>&1
