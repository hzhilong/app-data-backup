...
if not exist "!backup%pos%!\FMStorage.list" (
    echo 缺少配置文件 FMStorage.list
    call :failed
)

copy /y "!backup%pos%!\FMStorage.list" "%installLocation%\" >nul || call :failed
xcopy /e/y/i/f "!backup%pos%!\Storage" "%installLocation%\Storage" >nul 2>&1
echo ------------------------------------------
