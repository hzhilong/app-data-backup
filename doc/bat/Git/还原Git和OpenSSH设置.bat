...
if not exist "!backup%pos%!\.gitconfig" (
    echo 缺少配置文件 .gitconfig
    call :failed
)

copy /y "!backup%pos%!\.gitconfig" "%userprofile%\" >nul || call :failed
xcopy /e/y/i/f "!backup%pos%!\.ssh" "%userprofile%\.ssh" >nul 2>&1
echo ------------------------------------------
