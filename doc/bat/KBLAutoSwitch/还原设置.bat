...
if "%installLocation%"=="" (
    ...
    if exist "D:\software\%app_name%" ( set "installLocation=D:\software\%app_name%")
)
...
if not exist "!backup%pos%!\KBLAutoSwitch.ini" (
    echo 缺少配置文件 KBLAutoSwitch.ini
    call :failed
)

copy /y "!backup%pos%!\KBLAutoSwitch.ini" "%installLocation%\" >nul || call :failed
echo ------------------------------------------
