...
md "%dir%"
if not exist "%appdata%\%app_name%" (
    echo 缺少配置文件
    call :failed
)

xcopy /e/y/i/f "%appdata%\%app_name%" "%dir%\%app_name%" >nul || call :failed
xcopy /e/y/i/f "%installLocation%\Plugins" "%dir%\Plugins" >nul 2>&1
xcopy /e/y/i/f "%installLocation%\Languages" "%dir%\Languages" >nul 2>&1
