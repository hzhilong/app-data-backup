...
md "%dir%"
if not exist "%appdata%\%app_name%\preferences.json" (
    echo 缺少配置文件 preferences.json
    call :failed
)

copy /y "%appdata%\%app_name%\preferences.json" "%dir%\" >nul 2>&1 || call :failed
copy /y "%appdata%\%app_name%\recently-used-documents.json" "%dir%\" >nul 2>&1
copy /y "%appdata%\%app_name%\window-state.json" "%dir%\" >nul 2>&1 
copy /y "%appdata%\%app_name%\dataCenter.json" "%dir%\" >nul 2>&1
copy /y "%appdata%\%app_name%\keybindings.json" "%dir%\" >nul 2>&1
