...
if not exist "!backup%pos%!\preferences.json" (
    echo 缺少配置文件 preferences.json
    call :failed
)

md "%appdata%\%app_name%" >nul 2>&1
copy /y "!backup%pos%!\preferences.json" "%appdata%\%app_name%\" >nul || call :failed
copy /y "!backup%pos%!\window-state.json" "%appdata%\%app_name%\" >nul 2>&1
copy /y "!backup%pos%!\recently-used-documents.json" "%appdata%\%app_name%\" >nul 2>&1
copy /y "!backup%pos%!\dataCenter.json" "%appdata%\%app_name%\" >nul 2>&1
copy /y "!backup%pos%!\keybindings.json" "%appdata%\%app_name%\" >nul 2>&1
echo ------------------------------------------
