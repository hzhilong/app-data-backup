...
if not exist "!backup%pos%!\mirroid_conf.ini" (
    echo 缺少配置文件 mirroid_conf.ini
    call :failed
)

md "%localappdata%\%app_name%" >nul 2>&1
copy /y "!backup%pos%!\mirroid_conf.ini" "%localappdata%\%app_name%\" >nul || call :failed
echo ------------------------------------------
