...
if not exist "!backup%pos%!\Notepad3" (
    echo 缺少配置文件
    call :failed
)

md "%appdata%\Rizonesoft\%app_name%" >nul 2>&1
xcopy /e/y/i/f "!backup%pos%!\%app_name%" "%appdata%\Rizonesoft\%app_name%" >nul || call :failed
echo ------------------------------------------
