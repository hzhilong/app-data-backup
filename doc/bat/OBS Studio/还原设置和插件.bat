...
set installLocation=!installLocation:~0,-10!
...
if not exist "!backup%pos%!\%app_name%" (
    echo 缺少配置文件
    call :failed
)

md "%appdata%\obs-studio" >nul 2>&1
xcopy /e/y/i/f "!backup%pos%!\%app_name%" "%appdata%\obs-studio" >nul || call :failed
xcopy /e/y/i/f "!backup%pos%!\obs-plugins" "!installLocation!\obs-plugins" >nul || call :failed
xcopy /e/y/i/f "!backup%pos%!\data\obs-plugins" "!installLocation!\data\obs-plugins" >nul || call :failed
echo ------------------------------------------
