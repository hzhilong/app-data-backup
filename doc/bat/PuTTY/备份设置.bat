...
echo ------------------------------------------
echo 正在查找 %app_name% 的注册表数据...
reg query "HKEY_CURRENT_USER\SOFTWARE\SimonTatham\PuTTY" >nul 2>&1 || ( call :not_installed_then_exit "%app_name%" )
echo 存在注册表项：HKEY_CURRENT_USER\SOFTWARE\SimonTatham\PuTTY
echo ------------------------------------------
...
reg export "HKEY_CURRENT_USER\SOFTWARE\SimonTatham\PuTTY" "%dir%\backup.reg" >nul || call :failed
