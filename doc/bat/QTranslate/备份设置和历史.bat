...
md "%dir%"
if not exist "%appdata%\%app_name%\Options.json" (
    echo 缺少配置文件 Options.json
    call :failed
)

copy /y "%appdata%\%app_name%\Options.json" "%dir%\" >nul 2>&1 || call :failed
copy /y "%appdata%\%app_name%\History.json" "%dir%\" >nul 2>&1 || call :failed
