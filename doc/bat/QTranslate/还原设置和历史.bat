...
if not exist "!backup%pos%!\Options.json" (
    echo 缺少配置文件 Options.json
    call :failed
)

md "%appdata%\%app_name%" >nul 2>&1
copy /y "!backup%pos%!\Options.json" "%appdata%\%app_name%\" >nul || call :failed
copy /y "!backup%pos%!\History.json" "%appdata%\%app_name%\" >nul || call :failed
echo ------------------------------------------
