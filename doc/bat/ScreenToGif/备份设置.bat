...
if exist "%installLocation%\Settings.xaml" (
    echo 配置文件路径：%installLocation%\Settings.xaml
    md "%dir%\local"
    copy /y "%installLocation%\Settings.xaml" "%dir%\local\" >nul 2>&1 || call :failed
) ^
else if exist "%appdata%\%app_name%\Settings.xaml" (
    echo 配置文件路径：%appdata%\%app_name%\Settings.xaml
    copy /y "%appdata%\%app_name%\Settings.xaml" "%dir%\" >nul 2>&1 || call :failed
) ^
else (
    echo 缺少配置文件 Settings.xaml
    call :failed
)
