...
if exist "!backup%pos%!\local\Settings.xaml" (
    copy /y "!backup%pos%!\local\Settings.xaml" "%installLocation%\" >nul || call :failed
) ^
else if exist "!backup%pos%!\Settings.xaml" (
    md "%appdata%\%app_name%" >nul 2>&1
    copy /y "!backup%pos%!\Settings.xaml" "%appdata%\%app_name%\" >nul || call :failed
) ^
else (
