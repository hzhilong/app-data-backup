...
if not exist "!backup%pos%!\SumatraPDF-settings.txt" (
    echo 缺少配置文件 SumatraPDF-settings.txt
    call :failed
)

md "%localappdata%\%app_name%" >nul 2>&1
copy /y "!backup%pos%!\SumatraPDF-settings.txt" "%localappdata%\%app_name%\" >nul || call :failed
echo ------------------------------------------
