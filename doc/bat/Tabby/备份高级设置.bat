...
md "%dir%"
if not exist "%appdata%\%app_name%\config.yaml" (
    echo 缺少配置文件 config.yaml
    call :failed
)

copy /y "%appdata%\%app_name%\config.yaml" "%dir%\" >nul 2>&1 || call :failed
copy /y "%appdata%\%app_name%\config.yaml.backup" "%dir%\" >nul 2>&1
copy /y "%appdata%\%app_name%\Preferences" "%dir%\" >nul 2>&1
copy /y "%appdata%\%app_name%\window.json" "%dir%\" >nul 2>&1
