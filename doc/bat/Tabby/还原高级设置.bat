...
if not exist "!backup%pos%!\config.yaml" (
    echo 缺少配置文件 config.yaml
    call :failed
)

md "%appdata%\%app_name%" >nul 2>&1
copy /y "!backup%pos%!\config.yaml" "%appdata%\%app_name%\" >nul || call :failed
copy /y "!backup%pos%!\Preferences" "%appdata%\%app_name%\" >nul
copy /y "!backup%pos%!\window.json" "%appdata%\%app_name%\" >nul
echo ------------------------------------------
