...
set app_name=karasubonk_bilibili
echo ------------------------------------------
...
if not exist "!backup%pos%!\data.json" (
    echo 缺少配置文件
    call :failed
)
md "%APPDATA%\%app_name%\throws" >nul 2>&1
xcopy /e/y/i/f "!backup%pos%!\throws" "%APPDATA%\%app_name%\throws" >nul || call :failed
copy /y "!backup%pos%!\data.json" "%APPDATA%\%app_name%\" >nul || call :failed
