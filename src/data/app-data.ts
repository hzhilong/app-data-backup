...
export type AppData<T> = {
  ...
  getList: <Q extends Record<string, QueryParam> = Record<string, QueryParam>>(
    queryParams?: QueryParams<Q>,
  ) => Promise<T>
  // 刷新列表数据（重写初始化并更新数据库和缓存）
  refreshList: <Q extends Record<string, QueryParam> = Record<string, QueryParam>>(
    queryParams?: QueryParams<Q>,
...
export function getAppData<T>(
  ...
  const refreshList = async <Q extends Record<string, QueryParam> = Record<string, QueryParam>>(
    queryParams?: QueryParams<Q>,
  ...
  const getList = async <Q extends Record<string, QueryParam> = Record<string, QueryParam>>(
    queryParams?: QueryParams<Q>,
  ): Promise<T> => {
    try {
      loading.value = true
